...
describe('ZOtp', () => {
  test('Create a zotp', async () => {
    ...
    expect(otp.keyUri(secretKey)).not.toBe(undefined)
    uri = otp.keyUri(secretKey)
    console.log('Created the otp: ' + otp.keyUri(secretKey))
  })

  test('Verify - success', async () => {
    const otp = new ZOtp(secretKey, HashAlgorithms.HMACSHA1, 30)
    const res = otp.generate()
    console.log(`client=${uri}, new=${otp.keyUri(secretKey)}`)
